...
cacheSolve <- function(x, ...) {
  ...
  m <- x$getsolve() ##verifies matrix inverse is stored and is not null
  if(!is.null(m)) {
    message("getting cached data")
    return(m) ##if inverse is not null, displays message and returns inverse
  }
  data <- x$get() ##gets matrix stored by makeCacheMatrix
  m <- solve(data, ...) ##computes the inverse
  x$setsolve(m) ##stores value of inverse in the object already created
  m
}
